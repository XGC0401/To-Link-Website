...
export function EventsScreen() {
  return (
    <FeatureShell
      description="Community-hosted events can be discovered here and joined with participant details that feed into the calendar system."
      title="Events"
    >
      <div className="grid h-full gap-4 overflow-y-auto pr-1 xl:grid-cols-2">
        {nearbyCommunities.map((item) => (
          <article key={item.id} className="rounded-[28px] border border-border bg-panel-strong p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-accent-strong">Community Event</p>
