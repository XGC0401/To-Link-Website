...
async function sendCodeEmail(email: string, code: string): Promise<SendCodeResult> {
  ...
  if (isRenderFreeSmtpPortBlocked(smtp.port)) {
    throw new Error(
      "Render free web services cannot send SMTP traffic on ports 25, 465, or 587. Please configure a mail relay that accepts outbound SMTP on an alternative port, or use a transactional email provider with an HTTPS API.",
    );
  }
